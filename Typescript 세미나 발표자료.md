...
이후 인터넷 붐이 일어났고, 사람들은 또다른 프로그래밍 언어를 만들기보다는 이미 만들어져 있는 자바스크립트를 써야겠다고 결정하게 되었습니다. -> 인터넷 붐, 자바스크립트 사용량 증가에 대한 근거 부족
...
분석해볼 소스 코드는 현재 담당하고 있는 개인화 서비스 프로젝트에서 사용하고 있는 IobigoAudioElement 입니다.

### 코드 분석

#### 타입과 인터페이스

가장 먼저 살펴볼 부분은 IObigoAudioElement가 인터페이스로 정의되어있다는 점입니다. 타입스크립트에서 타입을 구축하는 구문에는 Type alias과 인터페이스가 있습니다. 둘은 굉장히 비슷하며, 대부분의 경우 동일하게 동작합니다. 하지만 두 가지 차이점이 존재하는데요, 예시를 통해 살펴보도록 하겠습니다.

앞에 보시면 BirdType 이라는 타입이 정의되어있습니다. 해당 타입은 wings, 날개라는 속성에 2라는 값을 반드시 가지고 있어야 합니다. 이제 BirdType을 다른 타입과 인터페이스를 확장하는데 사용해보겠습니다.

Owl 이라는 BirdType에서 확장된 타입을 정의해보도록 하겠습니다. Owl은 기본적으로 BirdType의 특성을 갖고, 야행성 속성에 대한 값을 추가적으로 갖고 있습니다.

확장을 위해서 인터섹션 타입을 사용하는데, & 키워드를 사용합니다. 여기서 인터섹션 타입이란 여러 타입을 모두 만족하는 하나의 타입을 의미합니다.

___

기본적으로 타입스크립트는 구조적 타입 시스템이기 때문에 타입과 인터페이스를 교차사용하는 것이 가능합니다. Chicken 이라는 인터페이스를 정의할 때 확장을 위해 extends 라는 키워드를 사용했고, 인터페이스가 아닌 Type alias인 BirdType을 확장하는데 교차 사용한 것을 확인할 수 있습니다.

두 번째 차이점은 Type Alias은 닫혀 있고, Interface는 열려 있다는 점입니다. 예시를 통해 살펴보겠습니다. 타입 alias 은 한 번 정의하고 나서 다시 정의하는 것이 불가능합니다. 동일한 타입으로 간주되어 중복된 타입이라는 오류 메시지가 뜨는 것을 확인할 수 있습니다.

하지만 인터페이스의 경우에는 두 번째 선언을 통해 재정의가 가능합니다. tasty 라는 속성에 true 라는 값을 추가해 재정의를 해보겠습니다. 밑에 chicken interface를 갖는 chicken 변수에 오류 메시지가 뜬 걸 확인할 수 있습니다.

이렇게 두 가지 차이점이 있습니다. 공식 문서에서는 인터페이스가 좀 더 자세한 오류 메시지를 제공하기 때문에 인터페이스 사용을 권장하고 있습니다. 하지만 최근 업데이트에 의해 Type Alias 에서도 동일하게 자세한 오류 메시지를 제공하고 있습니다.

___

따라서 본인이 어떤 목적을 가지고 있느냐에 따라 Type Alias와 인터페이스를 구분해 사용하는 것이 가장 합리적이라고 생각합니다.

먼저 Type Alias는 단순히 Type을 정의하고 제한하는 목적일 때 적합합니다. 상속이나 확장이 불필요하고 단순 원시 값으로 사용되는 경우에 사용합니다. Type Alias는 심플하지만 확장성에서 제한이 있습니다.

만약 객체지향프로그래밍의 객체를 구현하는 경우라면 interface를 사용하는 것이 좋습니다.

___

개인화 서비스에서도 객체지향프로그래밍을 구현하고자 인터페이스를 사용했다고 생각합니다. 화면을 보시겠습니다. ObigoAudioElement에서 IObigoAudioElement를 implements 하고 있는 걸 볼 수 있습니다. 이는 객체지향언어의 인터페이스 특성을 사용한 것입니다.

이후 재사용을 위해서뿐만 아니라 프로젝트 규모상 객체 지향 프로그래밍으로 프로젝트를 구현하는 것이 효과적이며, 이를 위해서 개인화 서비스 프로젝트에서 interface를 사용한 것은 올바른 선택이라고 생각합니다.

___

두 번째로 살펴볼 부분은 리턴값이 any 인 부분입니다. 이 부분은 any 대신에 object 타입을 사용하는 것이 좋다고 생각합니다.

getTimeInfo 라는 메서드는 any 타입을 반환하고 있는데, 실제로 메서드가 구현된 클래스로 가서 확인해보면 해당 메서드는 객체를 반환하고 있습니다. 

그리고 any 는 타입이라기 보다는 타입스크립트의 강제성에서 벗어나 다시 자바스크립트의 자유도를 되찾는 일종의 탈출장치 라고 보는 편이 더 맞다고 생각합니다. 공식문서에서도 다음과 같이 표현하고 있습니다.

정리하자면 any 타입은 타입스크립트로 하여금 특정 줄의 코드가 문제 없다고 인식시킬 때 사용하면 효과적이라는 뜻입니다. 즉, any는 @ts-ignore 주석을 추가하는 것과 같다고 할 수 있습니다.

이 기능은 처음 자바스크립트 프로젝트를 타입스크립트로 마이그레이션할 때는 유용할 수 있습니다. 하지만 any 를 사용하는 것은 결국 일정 부분에 대한 타입 검사를 하지 않도록 설정하는 것이기 때문에 완전한 타입스크립트 프로젝트를 추구한다면 any 사용은 지양하는 것이 좋습니다.

___

지금까지 타입스크립트에 대한 개요와 더불어 개인화 서비스 프로젝트의 소스 코드 일부분에 대해 분석해봤습니다. 짧은 코드에 대한 분석이었지만 타입스크립트에 대해 학습을 하면서 궁금했던 부분에 대한 고찰이 가능할 것 같아서 해당 코드를 분석하게 되었습니다. 
...
- 노마드코더 자바스크립트, 타입스크립트 강의 내용 일부
- 인프런 캡틴판교 타입스크립트 강의 내용 일부